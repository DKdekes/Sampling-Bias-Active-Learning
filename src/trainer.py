...
def train(logger, opt, dset, epoch, data_logger):
    ...
    y_prob_test = np.zeros((len(dset.X_test), opt.num_classes))*1.0
    y_feat = None
    ...
    for i in range(opt.num_ensemble):
        ...
        y_prob_test += model.predict_proba_(dset, train=False)
        y_feat = model.get_features_(dset.X)

        if not os.path.exists(opt.logpath + opt.exp_name + '/pretrained/'):
            os.makedirs(opt.logpath + opt.exp_name + '/pretrained/')
    ...
    y_prob_test /= (1.0*opt.num_ensemble)
    return y_prob_all, y_prob_test, y_feat
